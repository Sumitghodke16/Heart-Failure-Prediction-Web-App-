...
st.image("https://img.freepik.com/free-vector/detailed-vector-illustration-human-heart_1308-180437.jpg",
...
serium_sodium = st.number_input("Serum Sodium", min_value=0, max_value=200, value=135)
sex = st.selectbox("Sex (1=Male and 0=Female)", [0, 1])
smoking = st.selectbox("Smoking", [0, 1], format_func=lambda x: "Yes" if x == 1 else "No")
time = st.number_input("Time (Follow-up period in days)", min_value=0, max_value=300, value=100)

if st.button("Predict"):
    heart_input = np.array([[age, anaemia, creatinine, diabetes, ef, hbp, platelets, serum_creatinine, 
                             serium_sodium, sex, smoking, time]])
    scaled = heart_scaler.transform(heart_input)
    result = heart_model.predict(scaled)
    if result[0] == 1:
        st.success("Heart Failure Event")
    else:
        st.success("He is safe")